...
class Select extends Component {
  ...
  render() {
    const { selectedValue, treeData, isOpen, defaultValue, labelPlaceholder, inputPlaceholder, noContentText, disabled, allText } = this.state;
    ...
    return (
      <div className={`r-multiple-select-list ${className}`}>
        <div className={`r-label-list ${disabled ? 'r-label-list-disabled' : ''}`} onClick={disabled ? null : this.onInputClick}>
          {selectedValue.length ? selectedValue.length === treeData.length ? allText : selectedValue.map(item => item.label).join(',') : labelPlaceholder}
